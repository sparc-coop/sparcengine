...
async function saveTextNodesAsync(nodes) {
    ...
    db.sync(remote).on('change', function (info) {
...
function replaceText(id) {
    ...
    if (node) {
        db.get(id).then(function (doc) {
            console.log('Updating text node:', doc.Text, 'for language:', doc.LanguageId);
            node.nodeValue = doc.Text; // update the text node with the new content
        }).catch(function (err) {
            console.log('oh no', err);
        });
    } else {
        console.log('No node found for ID:', doc.id);
    }
...
async function saveKoriTextContentAsync(node) {
    ...
    const id = MD5(text + ':' + languageId);
    const eventualId = MD5(text + ':' + navigator.language);

    if (nodeIndex[eventualId]) {
        replaceText(eventualId);
        return;
    }
    ...
    nodeIndex[eventualId] = node; // store the node reference by its eventual MD5 hash
